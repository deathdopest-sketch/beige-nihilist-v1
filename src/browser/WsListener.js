 ...
 *   onJoin(roomName, nick, handle, username, mod)   ← mod = 0-4 permission level
 *   onJoined(roomName, nick, handle, username)      ← bot's own entry event
 *   onLeave(roomName, nick, handle)
 *   onNickChange(roomName, oldNick, newNick, handle)
 *   onUserList(roomName, users[])                   ← each user includes mod field
 *   onBan(roomName, nick, handle, context[])        ← user was banned; context = last 20 frames
 *   onKick(roomName, nick, handle, context[])       ← user was kicked
 *   onMute(roomName, nick, handle)                  ← user was muted
 *   onSysMsg(roomName, text)                        ← server system message
 *   onCamOn(roomName, handle)          ← user started broadcasting
 *   onCamOff(roomName, handle)         ← user stopped broadcasting
 *   onProducers(roomName, producers)   ← full snapshot of active streams
 *   onYouTube(roomName, data)          ← YouTube player event
 *   onNewFrameType(roomName, type, frame) ← first time a new stumble type is seen
...
class WsListener {
  constructor(roomName, page, selfNick, callbacks = {}, logger) {
    ...
    // Circular frame buffer — last 50 frames for pre-moderation context (WSMonitor-style)
    this._frameBuffer    = [];
  ...
  async start() {
    try {
      ...
      // Block source map fetches — StumbleChat CSP rejects them anyway, this stops the noise
  ...
  async injectRelay() {
    try {
      ...
      // Proactively resolve our own handle from DOM if 'joined' event was missing it.
      // StumbleChat sometimes sends joined with no fields (seen in meatspace room).
      if (!this._selfHandle) {
        try {
          const selfNick = this.selfNick;
          const domHandle = await this.page.evaluate((nick) => {
            for (const li of document.querySelectorAll('#userlist li[user-id], ul.userlist li[user-id], li.bar[user-id]')) {
              const n = li.querySelector('span.nickname, span.username')?.textContent?.trim() || '';
              if (n.toLowerCase() === nick.toLowerCase()) return li.getAttribute('user-id');
            }
            return null;
          }, selfNick);

          if (domHandle) {
            this._selfHandle = String(domHandle);
            this._nickMap.set(this._selfHandle, selfNick);
            this.log?.info(`[WS:${this.roomName}] Self-handle resolved from DOM scan: ${this._selfHandle} (${selfNick})`);
          } else {
            this.log?.warn(`[WS:${this.roomName}] Could not resolve self-handle from DOM — self-echo guard inactive`);
          }
        } catch (e) {
          this.log?.warn(`[WS:${this.roomName}] DOM self-handle scan failed: ${e.message}`);
        }
      }
    } catch (e) {
  ...
  _bindEvents(cdp) {
    cdp.on('Network.webSocketCreated', ({ requestId, url }) => {
      ...
      if (info.isStumbleChat) {
        const isReconnect = !!this._wsRequestId;
    ...
    cdp.on('Network.webSocketFrameReceived', ({ requestId, response }) => {
      ...
      try {
        ...
        if (msg.stumble) {
          ...
          if (msg.username)  diag.username  = msg.username;
          if (msg.users)     diag.userCount = msg.users.length;
          if (msg.producers) diag.producers = msg.producers;
          // Log all fields for lesser-known event types so we can learn their structure
          if (['youtube', 'producers'].includes(msg.stumble)) {
            this.log?.debug(`[WS:${this.roomName}] RECV ${JSON.stringify(msg)}`);
          } else {
  ...
  _dispatch(msg) {
    ...
    // Pure WebRTC signalling noise — no useful application data
    if (t === 'publish') return;
    ...
    switch (t) {
      ...
      case 'msg': {
        ...
        if (!nick) {
          // Unknown handle: trigger DOM bootstrap for eventual resolution.
          // Still process the message with a placeholder nick so commands work.
          this.cb.onUnknownHandle?.(this.roomName, h);
          // Don't drop: fire with placeholder so .commands still route correctly.
          // The placeholder will be replaced once the handle is resolved via join/userlist.
          const placeholder = `user_${h}`;
          if (msg.text?.startsWith('.')) {
            this.cb.onMessage?.(this.roomName, placeholder, msg.text, h);
          }
          break;
        }
        // Skip the bot itself
        ...
        if (msg.nick) this._nickMap.set(h, msg.nick);
      ...
      case 'joined': {
        ...
        if (Array.isArray(msg.userlist) && msg.userlist.length > 0) {
          for (const u of msg.userlist) {
            if (u.handle && u.nick) this._nickMap.set(String(u.handle), u.nick);
          }
      ...
      case 'nick': {
        ...
        const h = msg.handle ? String(msg.handle) : null;
        // Capture old nick from map BEFORE overwriting
        const oldNick = msg['old-nick'] || (h ? this._nickMap.get(h) : undefined) || '?';
        if (h && msg.nick) this._nickMap.set(h, msg.nick);
        ...
        this.cb.onNickChange?.(this.roomName, oldNick, msg.nick, h);
        break;
      }
      ...
      case 'userlist': {
        if (Array.isArray(msg.users)) {
          ...
          for (const u of msg.users) {
            if (u.handle && u.nick) { this._nickMap.set(String(u.handle), u.nick); mapped++; }
          }
          this.log?.info(`[WS:${this.roomName}] Userlist: ${msg.users.length} users, ${mapped} mapped`);
          // Normalise: ensure each user entry carries username if present
          const users = msg.users.map(u => ({ ...u, username: u.username || null }));
          this.cb.onUserList?.(this.roomName, users);
      ...
      case 'subscribe': {
        // subscribe = user started broadcasting their webcam.
        if (!msg.handle) break;
        const h = String(msg.handle);
        if (!this._nickMap.has(h)) this.cb.onUnknownHandle?.(this.roomName, h);
        this.cb.onCamOn?.(this.roomName, h);
        break;
      }

      case 'unsubscribe': {
        if (!msg.handle) break;
        this.cb.onCamOff?.(this.roomName, String(msg.handle));
        break;
      }
